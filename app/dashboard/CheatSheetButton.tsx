  ...
  return (
    <>
      <button 
        ...
        disabled={isLoading}
        className="mt-4 w-full flex items-center justify-center gap-2 py-2 bg-nord-8/10 hover:bg-nord-8/20 text-white rounded-lg text-sm font-medium transition-colors"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setIsOpen(false); }}>
            ...
            >
              <div className="p-4 border-b border-nord-3 flex justify-between items-center bg-nord-1">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <FileText className="w-5 h-5 text-white" />
                  AI Cheat Sheet
                </h2>
                <div className="flex gap-2">
                  <button onClick={handleDownload} className="px-3 py-1.5 bg-nord-14 hover:bg-nord-14 rounded-lg text-sm text-white flex items-center gap-2 transition-colors">
                    <DownloadCloud className="w-4 h-4" /> Download .md
                  </button>
                  <button onClick={() => setIsOpen(false)} className="px-3 py-1.5 bg-nord-2 hover:bg-white/20 rounded-lg text-sm text-white transition-colors">
                    Close
                  </button>
                </div>
              </div>
