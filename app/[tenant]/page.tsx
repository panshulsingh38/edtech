...
export default async function TenantPage({ params }: { params: { tenant: string } }) {
  ...
  return (
    <div className="min-h-screen text-white flex flex-col" style={{ backgroundColor: '#0a0a0f' }}>
      {/* Dynamic Header */}
      <header className="px-8 py-6 flex items-center justify-between border-b border-white/10" style={{ borderBottomColor: `${tenant.themeColor || '#4f46e5'}30` }}>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-8 mt-12 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-12">
          {tenant.tests.map(test => (
            <div key={test.id} className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 transition-all flex flex-col h-full">
          ...
          {tenant.tests.length === 0 && (
            <div className="col-span-full p-12 text-center border-2 border-dashed border-white/10 rounded-2xl text-gray-500">
