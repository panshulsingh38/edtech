  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4 mt-16">
        <div>
          <h2 className="text-3xl font-bold text-nord-6 mb-2">
            Activity Log
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {logs.map((log) => (
          <div 
            ...
            className="group cursor-pointer p-6 rounded-3xl bg-nord-1/50 border border-nord-3 hover:bg-nord-2/50 hover:border-nord-8/50 transition-all duration-300 flex flex-col"
          >
            <div className="flex items-start justify-between mb-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${log.type === 'SNAP_AND_SOLVE' ? 'bg-nord-9/20 border-nord-9/30' : 'bg-nord-12/20 border-nord-12/30'}`}>
                {log.type === 'SNAP_AND_SOLVE' ? (
                  <Camera className="w-6 h-6 text-nord-9" />
                ) : (
                  <BookOpen className="w-6 h-6 text-nord-12" />
                )}
              </div>
            </div>
      ...
      <AnimatePresence>
        {selectedLog && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
            ...
            >
              <div className="p-4 border-b border-nord-3 flex items-center justify-between bg-nord-1/50">
                <div className="flex items-center gap-3">
                  {selectedLog.type === 'SNAP_AND_SOLVE' ? (
                    <Camera className="w-5 h-5 text-nord-9" />
                  ) : (
                ...
                <button 
                  onClick={() => setSelectedLog(null)}
                  className="p-2 rounded-lg hover:bg-nord-2/50 transition-colors"
                >
