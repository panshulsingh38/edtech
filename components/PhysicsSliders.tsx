...
export default function PhysicsSliders({ variables, onChange, disabled }: PhysicsSlidersProps) {
  ...
  return (
    <div className="space-y-6 bg-white/5 p-6 rounded-xl border border-white/10 mt-4">
      <h4 className="text-sm font-bold tracking-wider uppercase text-indigo-400 mb-4">Interactive Variables</h4>
      {variables.map((v) => (
        <div key={v.name} className="flex flex-col gap-2">
          <div className="flex justify-between items-center">
            <label className="text-gray-300 font-medium capitalize">{v.name}</label>
            <span className="text-indigo-300 font-mono bg-indigo-500/10 px-2 py-1 rounded text-sm">
              {values[v.name]}
            </span>
          </div>
          <input
